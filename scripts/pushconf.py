#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Will download all config from /Hard%20Disk/Config/ or /DiskOnChip/Config/"""
...
# - create arg d2f (device to file) & f2d (file to device)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Will download all config from /Hard%20Disk/Config/ or /DiskOnChip/Config/')
    ...
    for subfolder in ["DiskOnChip", "Hard Disk", "Config"]:
    ...
    for filename in ftp.nlst(): # get filenames within the directory
        try:
            ftpcopy(filename, pathjoin(path, now(r'%Y-%m-%d-%H%M%S') + ' ' + filename))
            if filename.lower() in 'config.txt':
                ftpcopy(filename, pathjoin(path, filename))
        except:
